 ...
 * @author lenovo
 */
public class ScheduePDFEsignDAOImpl implements ScheduePDFEsignDAO {

    @Resource(name = "dataSource")
    private DataSource dataSource;

    @Resource(name = "repodataSource")
    private DataSource repodataSource;
    ...
    public void BillContributionPDFDownload(Document document, String billNo, String annexure, int month, int year) {
        ...
        String gcpfTotalFig = "";
        String gTotal = "";
        BillContributionRepotBean ecfb = new BillContributionRepotBean();

        try {
            con = dataSource.getConnection();

            ecfb = comonScheduleDao.getBillContributionRepotScheduleHeaderDetails(annexure, billNo);
            empList = comonScheduleDao.getBillContributionRepotScheduleEmpList(annexure, billNo, year, month);
            BillContributionRepotBean obj = null;
            if (empList != null && empList.size() > 0) {
                obj = new BillContributionRepotBean();
                for (int i = 0; i < empList.size(); i++) {
                    obj = (BillContributionRepotBean) empList.get(i);
