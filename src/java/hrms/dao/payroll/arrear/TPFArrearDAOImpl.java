 ...
 * @author DurgaPrasad
 */
public class TPFArrearDAOImpl implements TPFArrearDAO{
    ...
    public List getGPFScheduleAbstractList(String billno, int aqmonth, int aqyear) {
        ...
        try {
            ...
            String gpfAbstQry1 = "SELECT gpf_type, sum(ad_amt) amt FROM AQ_MAST AM INNER JOIN " + aqDtlsTbl + " AD on AM.aqsl_no = AD.aqsl_no WHERE BILL_NO='" + billno + "' "
                    + " AND AM.aq_month=" + aqmonth + " AND AM.aq_year=" + aqyear + " AND ((AD_CODE='TPF' and DED_TYPE='S') or (AD_CODE='GA' AND DED_TYPE='L') or (AD_CODE='GPDD' AND DED_TYPE='S') or "
    ...
    public ArrayList getEmpGpfDetails(String billNo) throws Exception {
        ...
        try {
            ...
            String gpfQuery = "select GP,cur_salary,ARRAY_TO_STRING(ARRAY[INITIALS, F_NAME, M_NAME,L_NAME], ' ') EMP_NAME,CUR_DESG,AQSL_NO,cpf_head,emp_mast.emp_id,GPF_NO,cur_basic"
                    ...
                    + " where bill_no=? and cpf_head > 0 and emp_mast.acct_type='TPF'";
            ...
            while (rs.next()) {
                if (rs.getString("EMP_NAME") != null && !rs.getString("EMP_NAME").equals("")) {
                    ...
                    if (slno % 8 == 0) {
                        gpfBean.setCarryForward(reportCarryForward(total, "TPF") + "");
                        ...
                        gpfBean.setBroughtForward(reportBroughtForward(total, "TPF") + "");
                    } else {
    ...
    public List getTPFScheduleAbstractList(String billno) {
        ...
        try {
            ...
            /*String gpfAbstQry1 = "SELECT gpf_type, sum(ad_amt) amt FROM AQ_MAST AM INNER JOIN " + aqDtlsTbl + " AD on AM.aqsl_no = AD.aqsl_no WHERE BILL_NO='" + billno + "' "
                    + " AND AM.aq_month=" + aqmonth + " AND AM.aq_year=" + aqyear + " AND ((AD_CODE='TPF' and DED_TYPE='S') or (AD_CODE='GA' AND DED_TYPE='L') or (AD_CODE='GPDD' AND DED_TYPE='S') or "
                    + " (AD_CODE='GPIR' AND DED_TYPE='S')) group by gpf_type order by gpf_type";*/
            String gpfAbstQry1 = "SELECT SUM(CPF_HEAD) AMT FROM (SELECT ARR_MAST.AQSL_NO,CPF_HEAD FROM ARR_MAST where BILL_NO = ?)temp";
            pst = con.prepareStatement(gpfAbstQry1);
            pst.setInt(1,Integer.parseInt(billno));
            rs = pst.executeQuery();
            while (rs.next()) {
                tpfBean = new TPFScheduleBean();

                tpfBean.setPfcode("");
                tpfBean.setTotalamt(rs.getString("AMT"));
                ...
                tpfBean.setCarryForward((int) amt + "");
                tpfBean.setCarryForward1((int) amt1 + "");
                
                tot = Numtowordconvertion.convertNumber((int) amt);
                tpfBean.setAmountInWords(tot.toUpperCase());
                tot1 = Numtowordconvertion.convertNumber((int) amt1);
                tpfBean.setAmountInWords1(tot1.toUpperCase());
                
                tpfAbstractList.add(tpfBean);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
