 ...
 * @author Manoj PC
 */
public class BreakinserviceDAOImpl implements BreakinserviceDAO {
    ...
    public List getBreakinserviceList(String empid) {
        ...
        BreakinserviceBean pbean = null;
        try {
            con = dataSource.getConnection();

            String sql = "SELECT leaveid,fdate,tdate,EN.doe,EN.not_id,EN.not_type, EN.auth,ordno,orddt,off_en FROM emp_notification EN"
                    + " INNER JOIN emp_leave EL ON EN.not_id = EL.not_id left outer join g_office on EN.off_code=g_office.off_code"
                    + " WHERE EL.emp_id = '" + empid + "' AND lsot_id = '04' ORDER BY leaveid desc";
            // System.out.println("Sql:"+sql);        
            pst = con.prepareStatement(sql);
            //pst.setString(1, empid);
            rs = pst.executeQuery();
            while (rs.next()) {
                pbean = new BreakinserviceBean();
    ...
    public BreakinserviceBean getEmpBISData(String leaveId) {
        ...
        try {
            ...
            String sql = "SELECT leaveid,fdate,tdate,s_days,EN.*,off_en FROM emp_notification EN"
                    + " INNER JOIN emp_leave EL ON EN.not_id = EL.not_id left outer join g_office on EN.off_code=g_office.off_code WHERE EL.leaveid = ? AND lsot_id = '04'";
            pst = con.prepareStatement(sql);
