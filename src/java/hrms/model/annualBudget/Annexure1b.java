/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package hrms.model.annualBudget;

/**
 *
 * @author Surendra
 */
public class Annexure1b {
    private String ddoCode="";
    private String hoa="";
    private String groupDetails="";
    private String incumbentName="";
    private String employeeId="";
    private String grade="";
    private String basicPay="";
    private String totalPayRequired="";
    private String arrear="";
    private String da="";
    private String hra="";
    private String oa="";
    private String rcm="";
    private String total="";
    
    private String basicPayNextYear="";
    
    public String getDdoCode() {
        return ddoCode;
    }

    public void setDdoCode(String ddoCode) {
        this.ddoCode = ddoCode;
    }

    public String getHoa() {
        return hoa;
    }

    public void setHoa(String hoa) {
        this.hoa = hoa;
    }

    public String getGroupDetails() {
        return groupDetails;
    }

    public void setGroupDetails(String groupDetails) {
        this.groupDetails = groupDetails;
    }

    public String getIncumbentName() {
        return incumbentName;
    }

    public void setIncumbentName(String incumbentName) {
        this.incumbentName = incumbentName;
    }

    public String getEmployeeId() {
        return employeeId;
    }

    public void setEmployeeId(String employeeId) {
        this.employeeId = employeeId;
    }

    public String getGrade() {
        return grade;
    }

    public void setGrade(String grade) {
        this.grade = grade;
    }

    public String getBasicPay() {
        return basicPay;
    }

    public void setBasicPay(String basicPay) {
        this.basicPay = basicPay;
    }

    public String getTotalPayRequired() {
        return totalPayRequired;
    }

    public void setTotalPayRequired(String totalPayRequired) {
        this.totalPayRequired = totalPayRequired;
    }

    public String getArrear() {
        return arrear;
    }

    public void setArrear(String arrear) {
        this.arrear = arrear;
    }

    public String getDa() {
        return da;
    }

    public void setDa(String da) {
        this.da = da;
    }

    public String getHra() {
        return hra;
    }

    public void setHra(String hra) {
        this.hra = hra;
    }

    public String getOa() {
        return oa;
    }

    public void setOa(String oa) {
        this.oa = oa;
    }

    public String getRcm() {
        return rcm;
    }

    public void setRcm(String rcm) {
        this.rcm = rcm;
    }

    public String getTotal() {
        return total;
    }

    public void setTotal(String total) {
        this.total = total;
    }

    public String getBasicPayNextYear() {
        return basicPayNextYear;
    }

    public void setBasicPayNextYear(String basicPayNextYear) {
        this.basicPayNextYear = basicPayNextYear;
    }
    
    
}
