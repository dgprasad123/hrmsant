...
<html>
    ...
    <body>
        ...
        <h1 style="margin:0px;font-size:18pt;color:#777777;border-bottom:1px solid #DADADA;padding-bottom:5px;">Manage Active Training</h1>
        <table border='0' cellspacing='1' width='100%' align='center' class='tblres table-bordered' style='font-size:10pt;margin-top:10px;'>
            ...
            <c:forEach items="${trainingList}" var="tl" varStatus="cnt">
                <tr style='background:#FFFFFF'>
                    <td>${cnt.index+1}</td>
                    <td>${tl.instituteName}</td>
                    <td>${tl.programName}</td>
                    <td>${tl.fromDate}</td>
                    <td>${tl.toDate}</td>
                    <td>${tl.duration}</td>
                    <td>${tl.capacity}</td>
                    <td>${tl.numApplicants}</td>
                    <td><a href="TAManageTrainingDetail.htm?trainingId=${tl.trainingProgramID}"><img src="images/view_detail_icon.png" alt="View Detail" width="20" /></a></td>
                    <td><a href="downloadApplicantExcel.htm?trainingId=${tl.trainingProgramID}" title="Download as Excel"><img src="images/excel_icon.png" alt="Download as Excel" height="20" /></a></td>
                </tr>
            </c:forEach>            
        </table>
    </div>
