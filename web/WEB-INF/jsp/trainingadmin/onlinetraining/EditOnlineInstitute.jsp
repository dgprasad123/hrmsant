        ...
        <div style="width:100%;margin:0px auto;"><h1 style="font-size:18pt;margin:0px;margin-bottom:10px;">Manage Online Institutes</h1>
            ...
            <form:form class="form-control-inline"  action="AddOnlineInstitute.htm" method="POST" commandName="TrainingInstituteForm">
                ...
                <table width="100%" class="institute_form" cellspacing="1" cellpadding="4" border="0" bgcolor="#EAEAEA" style="border:1px solid #CCCCCC;font-size:10pt;margin-top:10px;">
                    ...
                    <tr>
                        ...
                        <td><select name="outsideTerritory" id="outsideTerritory" size="1">
                                <option value="INSIDE"<c:if test = "${InstituteForm.outsideTerritory == 'INSIDE'}"> selected="selected"</c:if>>Inside State</option>
                                <option value="OUTSIDE_STATE"<c:if test = "${InstituteForm.outsideTerritory == 'OUTSIDE_STATE'}"> selected="selected"</c:if>>Outside State</option>
                                <option value="FOREIGN"<c:if test = "${InstituteForm.outsideTerritory == 'FOREIGN'}"> selected="selected"</c:if>>Foreign</option>
                            </select></td>
                        <td align="right"><span style="color:#FF0000;">*</span> Allow to apply:</td>
                        <td><select name="isAllowed" id="isAllowed" size="1" style="width:150px;">
                                <option value="">-Select-</option>
                                <option value="Y"<c:if test = "${InstituteForm.isAllowed == 'Y'}"> selected="selected"</c:if>>Allow</option>
                                <option value="N"<c:if test = "${InstituteForm.isAllowed == 'N'}"> selected="selected"</c:if>>Not Allow</option>
                            </select></td>                            
                    </tr> 
                    <tr>
                        <td align="right">Course Type:${InstituteForm.courseType}</td>
                        <td colspan="3"><select name="courseType" id="courseType" size="1">
                                <option value="">-Select-</option>
                                <option value="Paid"<c:if test = "${InstituteForm.courseType == 'Paid'}"> selected="selected"</c:if>>Paid</option>
                                <option value="Completely Free"<c:if test = "${InstituteForm.courseType == 'Completely Free'}"> selected="selected"</c:if>>Completely Free</option>
                                <option value="Free with Conditions"<c:if test = "${InstituteForm.courseType == 'Free with Conditions'}"> selected="selected"</c:if>>Free with Conditions</option>
                            </select></td>
                ...
                <p><strong>Department:</strong>
                    ...
                    <select name="allowed" id="allowed" size="1" onchange="javascript: searchResults();" style="width:150px;">
