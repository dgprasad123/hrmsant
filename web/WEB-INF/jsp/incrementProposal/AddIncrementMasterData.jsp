...
<html>
    <head>
        ...
        <title>Add Increment Proposal</title>
                ...
                if (confirm("Are you sure you want to remove this Employee?"))
                {
                    $.ajax({
                        ...
                        success: function(retVal) {
                            self.location = 'newProposalMaster.htm?action=Create Increment Proposal';
                        }
                    });
                }
            }
    ...
    <body>
        <form:form action="newProposalMaster.htm" commandName="IncrementProposal" method="post" id="frmIncrement">
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th width="5%">Date of present </br> increment</th>
                                    <th>Pay Commission</th>
                                    <th>Level</th>
                                    <th>Cell</th>

                                    <th width="5%">Future pay</th>
                                    <th>Last 1 Year Pay</th>
                                    <th width="5%">Remove</th>
                                </tr>
                            </thead>
                            <tbody>
                                <c:forEach items="${ProposalList}" var="list" varStatus="counter">
                                    <tr>
                                        <td>${counter.count}</td>
                                        <td>${list.empId}
                                            <input type="hidden" name="emplist" value="${list.empId}" />
                                            <input type="hidden" id="present_pay_${list.empId}" value="${list.presentpay}" />
                                            <input type="hidden" id="previous_cell_${list.empId}" value="${list.matrixCell}" />  
                                        </td>
                                        <td>${list.gpfno}</td>
                                        <td>${list.empname}</td>
                                        <td>${list.post}</td>
                                        <td>${list.presentpay}</td>
                                        <td>${list.presentpaydate}</td>
                                        <td style="position:relative"><input type="text"  id="wef_date_${list.empId}" name="wefDate" value="${list.nextincr}" readonly="readonly" style="width:120px;" class="wDate form-control" /></td>
                                        <td><select name="payCommission" id="pay_commission_${list.empId}" size="1" onchange="javascript: getFuturePay('${list.empId}', ${list.presentpay})">
                                                <option value="0">-Select-</option>
                                                <option value="7"<c:if test = "${list.payCommission == '7'}"> selected="selected"</c:if>>7th</option>
                                                <option value="6"<c:if test = "${list.payCommission == '6'}"> selected="selected"</c:if>>6th</option>
                                                </select></td>
                                            <td><span id="sp_lvl_blk_${list.empId}"><select name="matrixLevel" id="matrix_level_${list.empId}" size="1" onchange="javascript: getCells('${list.empId}', this.value, ${list.presentpay}, '');">
                                                    <option value="0">-Select-</option>
                                                    <c:forEach var = "i" begin = "1" end = "30">
                                                        <option value="${i}"<c:if test = "${list.matrixLevel == i}"> selected="selected"</c:if>>${i}</option>
                                                    </c:forEach>                                                        
                                                </select></span></td>
                                        <td><div id="cell_blk_${list.empId}"><select name="matrixCell" id="matrix_cell_${list.empId}" size="1">
                                                    <option value='0'>-Select-</option>
                                                </select></div></td>                                            
                                        ...
                                        <td><a href="javascript:void(0)" onclick="javascript: deleteEmp('${list.empId}', 0)">Remove</a></td>
                                    </tr>
                                </c:forEach>
                                    <input type="hidden" id="main_counter" value="${ProposalList.size()}" />
                            </tbody>
                        </table>
                    </div>
                    <div class="panel-footer">
                        <input type="submit" name="action" class="btn btn-default" value="Add Employee"/>
                        <input type="submit" name="action" class="btn btn-default" value="Save Proposal"
                               <c:if test="${numProposals eq 0}">onclick="javascript:return false;"</c:if>
                               <c:if test="${numProposals > 0}">onclick="javascript:return validateSubmit();"</c:if>
                                   />
                               <input type="submit" name="action" class="btn btn-default" value="Back to Proposals"/> 
                        </div>
                    </div>
                </div>
        </form:form>
