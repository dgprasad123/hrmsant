...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            function SaveOR(aqSlNo) {
                var orVal = $("#txtOR" + aqSlNo).val();
                var taxBillNo = $("#billNo").val();
                if (orVal == '') {
                    alert("Please Enter Other Recovery Amount");
                    return false;
                } else {
                    $.ajax({
                        type: "GET",
                        url: "saveORArrMast.htm",
                        data: {aqslno: aqSlNo, billNo: taxBillNo, orAmt: orVal},
                        success: function(data) {
                            location.reload();
                        },
                        error: function() {
            ...
            function SaveDR(aqSlNo) {
                var drVal = $("#txtDR" + aqSlNo).val();
                var taxBillNo = $("#billNo").val();
                if (drVal == '') {
                    alert("Please Enter DDO Recovery Amount");
                    return false;
                } else {
                    $.ajax({
                        type: "GET",
                        url: "saveDRArrMast.htm",
                        data: {aqslno: aqSlNo, billNo: taxBillNo, drAmt: drVal},
                        success: function(data) {
                            location.reload();
                        },
                        error: function() {
                        ...
                        <thead>
                            <tr>
                                ...
                                <c:if test="${(BillSts < 2) or (BillSts == 4) or (BillSts == 8)}"> 
                                    ...
                                    <th width="8%">Other<br />Recovery</th>
                                    <th width="8%">DDO<br />Recovery</th>
                                    </c:if>
                        ...
                        <tbody>
                            <c:forEach items="${AqArrList}" var="arrAqMast" varStatus="cnt">    
                                <tr>
                                    ...
                                    <c:if test="${(BillSts < 2) or (BillSts == 4) or (BillSts == 8)}"> 
                                        <td><input name="incomeTaxAmt" id="txtAmt${arrAqMast.aqSlNo}" value="${arrAqMast.incomeTaxAmt}" maxlength="7" style="width:50%;" onkeypress="return onlyIntegerRange(event)" />
                                            <input type="button" value="Save" onclick="SaveIncomeTax('${arrAqMast.aqSlNo}')"/>
                                        </td>
                                        <td>
                                            <c:if test="${arrearPercent eq '30' || arrearPercent eq '20' || arrearPercent eq '15'}">
                                                <input name="cpfHead" id="txtCpf${arrAqMast.aqSlNo}" value="${arrAqMast.cpfHead}" maxlength="6" style="width:50%;" readonly="true"/>
                                            </c:if>
                                            <c:if test="${arrearPercent ne '30' && arrearPercent ne '20' && arrearPercent ne '15'}">
                                                <input name="cpfHead" id="txtCpf${arrAqMast.aqSlNo}" value="${arrAqMast.cpfHead}" maxlength="6" style="width:50%;" onkeypress="return onlyIntegerRange(event)" />
                                                <input type="button" value="Save" onclick="SaveCPF('${arrAqMast.aqSlNo}')"/>
                                            </c:if>
                                        </td>
                                        <td>
                                            <input name="ptAmt" id="txtPt${arrAqMast.aqSlNo}" value="${arrAqMast.pt}" maxlength="5" style="width:50%;" onkeypress="return onlyIntegerRange(event)" />
                                            <input type="button" value="Save" onclick="SavePT('${arrAqMast.aqSlNo}')"/>
                                        </td>
                                        <td>
                                            <input name="orAmt" id="txtOR${arrAqMast.aqSlNo}" value="${arrAqMast.otherRecovery}" maxlength="6" style="width:50%;" onkeypress="return onlyIntegerRange(event)" />
                                            <input type="button" value="Save" onclick="SaveOR('${arrAqMast.aqSlNo}')"/>
                                        </td>
                                        <td>
                                            <input name="drAmt" id="txtDR${arrAqMast.aqSlNo}" value="${arrAqMast.ddoRecovery}" maxlength="6" style="width:50%;" onkeypress="return onlyIntegerRange(event)" />
                                            <input type="button" value="Save" onclick="SaveDR('${arrAqMast.aqSlNo}')"/>
                                        </td>
                                    </c:if>

                                    <td>
                                        ${arrAqMast.arrear40 - (arrAqMast.incomeTaxAmt + arrAqMast.cpfHead + arrAqMast.pt + arrAqMast.otherRecovery + arrAqMast.ddoRecovery)}
                                    </td>
                                    <td>
                                    ...
                                    <c:if test="${(BillSts < 2) or (BillSts == 4) or (BillSts == 8)}"> 
                                        ...
                                        <td>
                                            <a href="javascript:giveFullArrear('${arrAqMast.aqSlNo}','${billNo}')" class="btn btn-default"><img src="images/action.png" height="20" alt="Full Arrear"/></a>
                                        </td>
                                    </c:if>
                                </tr>
                            </c:forEach>
