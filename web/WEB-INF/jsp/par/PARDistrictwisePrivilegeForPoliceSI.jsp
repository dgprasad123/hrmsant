...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function() {
                ...
                $('#datatable').DataTable();
                // Initialize select2 for searchable combo box
            ...
            function getOfficewiseprivilageAssignedList() {
                ...
                var url = 'getOfficeWiseAssignPrivilegeSiPar.htm?Offcode=' + tOffcode;
                $.getJSON(url, function(result) {
                    ...
                    $.each(result, function(i, field) {
                        ...
                        trhtml = trhtml + '<td align="center"><a href="#" onclick="removeOfficeWisePrivilageSiPar(\'' + field.spc + '\',\'' + field.postGroup + '\',\'' + tOffcode + '\')"><span class="glyphicon glyphicon-remove"></span>' + '</a></td>';
                        trhtml = trhtml + '</tr>';
            ...
            function assignOfficewisePrivilage() {
                ...
                tAuthorizationType = "S";
                tEmpPostGroup = $("input[name='emppostGroup']:checked").val();
                if (tSpc && tOffcode && tAuthorizationType) {
                    $.post("saveOfficewisePrivilage.htm", {spc: tSpc, offcodeforofficewise: tOffcode, authorizationType: tAuthorizationType, postGroup: tEmpPostGroup}, "json")
                            .done(function(data) {
                                if (data.msg == "Y") {
            ...
            function removeOfficeWisePrivilageSiPar(vSpc, vPostGroup, vOffcode) {
                $.post("removeSiParPrivilage.htm", {spc: vSpc, Offcode: vOffcode, postGroup: vPostGroup}, "json")
                    .done(function(data) {
                        getOfficewiseprivilageAssignedList();
    ...
    <div id="wrapper">
        ...
        <form:form action="DistrictWisePrivilegeForPoliceSiPar.htm" method="GET" commandName="cadre">
            <div id="page-wrapper">
                <div class="panel panel-default">
                    <h3 style="text-align:center;color:#0000FE"> <b> District Wise Privilege Officers List for PAR - SI </b> </h3>
                    <div class="form-group">&nbsp;&nbsp;
                        <button type="button" class="btn btn-primary" onclick="openSetPrivilegedOfficewiseWindow()"> <b> Set Privilege Office wise </b></button>
                        <!-- <input type="submit" name="action" value="Get Assigned PrivilegeListSi" class="btn btn-primary"> -->
                    </div>
                    <div class="panel panel-default">
                            ...
                            <table id="datatable" class="table table-bordered table-hover table-striped">
                                <thead class="tblTrColor">
                                    <tr>
                                        ...
                                        <th width="7%">District Name</th>
                                        <th width="25%">Office Name</th>
                                        <th width="7%">View Post Group</th>
                                        <th width="3%">Remove</th>
                                    </tr>
                                </thead>
                                <tbody>                                        
                                    <c:forEach items="${distWisePrivilegedList}" var="eachPerson" varStatus="cnt">
                                        <tr>
                                            <td>${cnt.index + 1}</td>
                                            <td>${eachPerson.priviligeAuhName},<br/><b style="color:#FF4500;">(${eachPerson.authHrmsId})</b></td>
                                            <td>${eachPerson.priviligeAuhDesignation}</td>
                                            <td>${eachPerson.distName}</td>
                                            <td>${eachPerson.officeName}</td>
                                            <td>${eachPerson.postGrp}</td>
                                            <td><a href="removeDistWisePrivilege4SiPar.htm?spc=${eachPerson.spc}&postGrp=${eachPerson.postGrp}"><span class="glyphicon glyphicon-remove"></span></a></td>
                                        </tr>
                                    </c:forEach>      
                                </tbody>
                            </table>
