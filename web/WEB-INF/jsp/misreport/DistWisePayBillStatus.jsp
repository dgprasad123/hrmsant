            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading" align="center" style="background-color: #868686;color: #ffffff;font-size: xx-large;">District Wise PayBill Report</div>
                        <div class="panel-body">
                            <div class="table-responsive">
                                <form:form role="form" action="getDistWisePayBillReport.htm" commandName="deptVacantPostList"  method="post">
                                    <table class="table table-bordered table-hover table-striped">
                                        <tr>
                                            <td align="right">Bill Type: </td>
                                            <td>
                                                <select name="typeofBill"  class="form-control" required="1">
                                                    <option value="PAY" <c:if test = "${not empty typeofBill && typeofBill=='PAY'}"> <c:out value='selected="selected"'/></c:if>>PAY</option>
                                                    <option value="OTHER_ARREAR" <c:if test = "${not empty typeofBill && typeofBill=='OTHER_ARREAR'}"> <c:out value='selected="selected"'/></c:if>>OTHER ARREAR</option>
                                                    <option value="ARREAR" <c:if test = "${not empty typeofBill && typeofBill=='ARREAR'}"> <c:out value='selected="selected"'/></c:if>>ARREAR 40 or 100 </option>
                                                    <option value="ARREAR_6" <c:if test = "${not empty typeofBill && typeofBill=='ARREAR'}"> <c:out value='selected="selected"'/></c:if>>ARREAR 10 or 60 </option>
                                                </select>   
                                            </td>
                                            ...
                                            <tr>
                                                <td colspan="6" align="center"><input type="submit" class="btn btn-primary" value="Submit"  /></td>
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover table-striped">
                                    ...
                                    <c:set var="ErrorPrepared" value="${0}" />
                                    <c:forEach items="${billDetails}" var="bgroup" varStatus="count">
                                        ...
                                        <c:set var="ErrorPrepared" value="${ErrorPrepared + bgroup.noofError}" />
                                        <tr>
                                            <td>${count.index + 1}</td>
                                            <td><a href="DistWiseOfficePayBill.htm?typeofBill=${typeofBill}&month=${selectmonth}&year=${selectyear}&dcode=${bgroup.deptCode}" target='blank'>${bgroup.departmentname}</a></td>
