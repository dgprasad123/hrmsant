            ...
            <div class="container-fluid">
                <div class="panel panel-default">
                    ...
                    <div style="font-weight:bold;font-size: 20px;text-align: center">
                        Please select Administrative Office to view Data
                    </div>
                    <div class="panel-body" style="height:400px; overflow: auto;">
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th>Sl No</th>
                                    <th>Administrative Office Name</th>
                                    <th>DDO Code</th>
                                    <th>Status</th>
                            ...
                            <tbody>
                                <c:forEach items="${aoofficelist}" var="offlist" varStatus="counter">
                                    <tr>
                                        <td>${counter.count}</td>
                                        <td><strong>${offlist.offName}</strong></td>
                                        <td>${offlist.ddoCode}</td>
                                        <td>
                                            <c:if test="${not empty offlist.aoaerid}">
                                                SUBMITTED
                                            </c:if>
                                            <c:if test="${empty offlist.aoaerid}">
                                                PENDING
                                            </c:if>
                                        </td>
                                        <td>
                                            <c:if test="${not empty offlist.aoaerid}">
                                                <a class="btn btn-primary" href="downloadaerPDFReportForScheduleIII.htm?aerId=${offlist.aoaerid}&financialYear=${selectedFiscalYear}" target="_blank">
                                                    <span class="fa fa-file-pdf-o" style="color:red"> </span> Download Schedule-II
                                                </a>
                                            </c:if>
                                            <a class="btn btn-danger" href="displayAERlistForAdministrativeAuthority.htm?btnAer=Search&aoOffCode=${offlist.offCode}&financialYear=${selectedFiscalYear}">
