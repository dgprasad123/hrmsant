<%-- 
    Document   : AnnualEstablishmentReportForCO
    Created on : Jun 11, 2019, 3:04:50 PM
    Author     : DurgaPrasad
--%>
    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function() {
                ...
                $('.openRevertModalverifier').on('click', function() {
      ...
      $('.dropdown-toggle').on('click', function() {
            ...
            function openEditPage(aerid) {
                self.location = "ScheduleIChangeAtReviewer.htm?btnAer=List&aerId=" + aerid;
            }
            ...
            <div class="container-fluid">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body" style="height:400px; overflow: auto;">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <c:if test="${not empty establish.revertReason}">
                                                <a href="javascript:void(0);" data-href="viewVerifierRevertReason.htm?aerId=${establish.aerId}" class="openRevertModalverifier">
                                        ...
                                        <td> &nbsp;        
                                            <c:if test="${not empty establish.aerId}">
                                                   ...
                                                   <!-- <div class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            ...
                                                            <li><a href="javascript:openDeclineModal('${establish.aerId}');">Decline</a></li>
                                                                <c:if test="${roleType eq 'REVIEWER' || roleType eq 'BO'}">
                                                                <li><a href="javascript:openEditPage('${establish.aerId}');">Edit</a></li>
                                                                </c:if>
                                                        </ul>
                                                    </div>-->
                                                                <select onchange="javascript: openPopups(this.value, '${establish.aerId}')" id="dropdown_new" class="form-select form-select-lg mb-3" style="background:#FFFFFF;border:1px solid #CCC;border-radius:5px;padding:5px;">
                                                ...
                                                <c:if test="${establish.showApproveLink eq 'N' && coaerid lt 1}">
                        ...
                        <script type="text/javascript">
                            function openPopups(val, aerId)
                            {
                                if(val == 'Approve')
                                {
                                    openApproveModal(aerId);
                                }
                                if(val == 'Decline')
                                openDeclineModal(aerId);
                              if(val == 'Edit')
                                openEditPage(aerId);
                    ...
                    <div class="panel-footer">
                        <c:if test="${coaerid lt 1}"> 
                            <c:if test="${roleType eq 'REVIEWER' || roleType eq 'BO'}">
                                <button type="submit" name="btnAer" value="View" class="btn btn-primary">View Consolidated Report</button>
                            </c:if>
                            <c:if test="${roleType eq 'VERIFIER' || roleType eq 'BO'}">
                                <c:if test="${empty command.coOffCode}">
                                    <button type="submit" name="btnAer" value="ViewAsVerifier" class="btn btn-primary">View Consolidated Report(As Verifier)</button>
                                </c:if>
                                <c:if test="${not empty command.coOffCode}">
                                    <form:hidden path="coOffCode"/>
                                    <a class="btn btn-primary" href="displayAERlistForControllingAuthority.htm?btnAer=GetCOData&financialYear=${command.financialYear}">Back</a>
                                    <button type="submit" name="btnAer" value="ViewAsVerifierMultipleCO" class="btn btn-primary">View Consolidated Report(As Verifier)</button>
                                </c:if>
                            </c:if>
                            (N:B:- Consolidated report will be shown based on Approve AER.  ) 
                        </c:if>
                        <c:if test="${coaerid gt 1}">
                            <c:if test="${not empty command.coOffCode}">
                                <a class="btn btn-primary" href="displayAERlistForControllingAuthority.htm?btnAer=GetCOData&financialYear=${command.financialYear}">Back</a>
                            </c:if>
                            <a class="btn btn-primary" href="downloadaerPDFReportForScheduleII.htm?aerId=${coaerid}&financialYear=${command.financialYear}" target="_blank">
